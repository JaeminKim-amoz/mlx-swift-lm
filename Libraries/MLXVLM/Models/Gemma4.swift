...
public final class Gemma4: Module, VLMModel, KVCacheDimensionProvider {
    ...
    /// Prepare with optional audio features.
    /// Audio features should be passed as mel spectrograms [B, T, 128] with an optional
    /// boolean mask [B, T] (true = padding/invalid).
    public func prepareWithAudio(
        _ input: LMInput, cache: [any KVCache],
        audioFeatures: MLXArray? = nil, audioMask: MLXArray? = nil
    ...
    public func sanitize(weights: [String: MLXArray]) -> [String: MLXArray] {
        ...
        // Filter audio weights only if no audio config
        ...
        // Filter clipping params for vision tower if not used
        if !config.visionConfiguration.useClippedLinears {
            sanitized = sanitized.filter { key, _ in
                if key.contains("vision_tower") {
                    return !key.contains("input_min")
        ...
        // Filter clipping params for audio tower if not used
        if let audioConfig = config.audioConfiguration, !audioConfig.useClippedLinears {
            sanitized = sanitized.filter { key, _ in
...
public struct Gemma4ProcessorConfiguration: Codable, Sendable {
    ...
    public init(from decoder: any Swift.Decoder) throws {
        ...
        // audio_token_id is in config.json but NOT in processor_config.json.
        // Default to 258881 (<|audio|>) which is the standard Gemma4 audio token.
